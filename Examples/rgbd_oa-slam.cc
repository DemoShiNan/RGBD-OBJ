...
int main(int argc, char **argv)
{
    ...
    string strAssociationFilename = string(argv[8]);
    ...
    while (1)
    {
        ...
        {
            filename = path_to_images + vstrImageFilenamesRGB[ni];
            // im = cv::imread(filename, cv::IMREAD_UNCHANGED);  // read image from disk
            imRGB = cv::imread(path_to_images+"/"+vstrImageFilenamesRGB[ni],CV_LOAD_IMAGE_UNCHANGED);
        ...
        cv::Mat m = SLAM.TrackRGBD(imRGB,imD,tframe,detections, false);
