...
public:
    ...
    void AddAssociatePlane(const long unsigned int& plane_id, const double& distance){
        associated_planes_id_dist_[plane_id] = distance;
    }
    std::unordered_map<long unsigned int,double> GetAssociatePlane() const{
        return associated_planes_id_dist_;
    }
    ...
    void Compute_plane_dist(Map* map);
...
public:
    ...
    //<id , dist>  
    std::unordered_map<long unsigned int,double> associated_planes_id_dist_;
};
