...
ObjectTrack::Ptr ObjectTrack::CreateNewObjectTrack(unsigned int cat, const BBox2& bbox, double score, const Matrix34d& Rt, unsigned int frame_idx, Tracking *tracker, KeyFrame *kf) {
    ...
    obj->associated_planes_id_dist_ = std::unordered_map<long unsigned int,double>();
    return obj;
}
...
//这里传入的map是Tracking线程提供的mpMap
void ObjectTrack::Compute_plane_dist(Map* map){
    const vector<MapPlane*> &vplanes = map->GetAllMapPlanes();
    if (vplanes.empty())
        return;
    Ellipsoid ell = map_object_->GetEllipsoid();
    Eigen::Vector3d center = ell.GetCenter();
    //compute dist
    std::vector<pair<long unsigned int,double>> id_dist;
    for(long unsigned int i=0 ; i < vplanes.size(); i++ ){
        cv::Mat plane = vplanes[i]->GetWorldPos();
        double dist = 0.0;
        //单位法向量点乘
        dist = abs(plane.at<float>(0, 0) * center.x() +
                   plane.at<float>(1, 0) * center.y() +
                   plane.at<float>(2, 0) * center.z() +
                   plane.at<float>(3, 0));
        id_dist.push_back(make_pair(vplanes[i]->mnId,dist));
    }
    //save and anking
    sort(id_dist.begin(),id_dist.end(),[](const pair<long unsigned int,double> &a,const pair<long unsigned int,double> &b) {
                                                                                return a.second<b.second;
                                                                            });
    //display
    cout<< "plane size: "<< id_dist.size()<<endl;
    //赋值associated_planes_id_dist;
    auto associated_planes_id_dist_ = GetAssociatePlane();
    for(auto it=0; it < id_dist.size();it++){
        if( associated_planes_id_dist_.size()>=5){
            break;
        }
        // associated_planes_id_dist_[id_dist[it].first] = id_dist[it].second;
        AddAssociatePlane(id_dist[it].first,id_dist[it].second);
        cout<<"plane_id : "<<id_dist[it].first<<"dist_value : "<<id_dist[it].second <<endl;
    }
}
/* example
plane id: 3
